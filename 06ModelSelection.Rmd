...
------------------------------
We will now use packages `leaps` to evaluate all the best- subset models.
```{r}
library(leaps)
...
It gives by default best-subsets up to size 8; lets increase that to 19 i.e. all variables
...
mincomp<-which.min(reg.summary$cp) #Identify the samllest component
points(mincomp,reg.summary$cp[10],pch=20,col="red")
```

There is a plot method for `regsubsets`object
```{r}
#Bad Cps have all the variables in!
plot(regfit.full, scale="Cp")  #Black squares mean that the variable is IN!  White are out!
coef(regfit.full, 10)
```
...
```{r}
#The variables are nested, each contains 
regfit.fwd=regsubsets(Salary~.,data=Hitters, nvmax=19, method="forward")
summary(regfit.fwd)
#Schematic plot, looks very similar to the one of base subsets.
...
dim(Hitters)
set.seed(1)
train=sample(?), 180, replace=FALSE)
train
regfit.fwd = regsubsets(Salary~., data=Hitters[train,], nvmax=19, method="forward")
```
Now we will make predictions on the observations not used fro training. We know there are 19 models, so we set up some vectors to record the errors. We have to do a bit of work here, because there is no predict method for `regsubsets`.
...
-----------------------------------
We will use a ten fold cross validation
```{r}
set.seed(11)
folds<-sample(rep(1:10, length=nrow(Hitters)))
folds
table(folds)
cv.errors=matrix(NA,10,19) #10 rows (folds) and 19 columns (Variables)
for(k in 1:10){
  #k is our foldfor the test set.
