...
Carseats=data.frame(Carseats, High)    #Append High to the dataframe
```
...
```{r}
tree.carseats<- tree(High~.-Sales, data=Carseats) #Take out sales cause is what we are predicting with "High".
...
```{r}
tree.carseats # A printout of the tree. Handy if you want to extract details for other purposes.
```

```{r}
set.seed(1011)
train=sample(1:nrow(Carseats),250)      #By default we sample without replacement.
...
(72+32)/150  #In this case it dropped a bit, we didnt get much for pruning
```
It has done about the same as our original tree. So pruning did not hurt us wrt misclassification errors, and gave us a simplier tree.

Trees do not give very good prediction errors, now its time to look random forests and boosting.

Random Forest and Boosting.
===================================

These methods use trees as building blocks to build more complex models. Here we will use the Boston housing data to explore random forests and boosting. These data are in `MASS` package.
...
Not too difficult! Although the test-error curve drops below the OOB curve, these are estimates based on data, and so have their own standard errors (which are typically quite large). Notice that the points at the end with `mtry=13`
correspond to bagging.
...
Also to tune shrinkage and depth.