...
```{r}
fitta = lm(wage~age+I(age^2)+I(age^3)+I(age^4), data=Wage)
summary(fitta)
```

Here `I()` is a *wrapper* function; we need it because `age^2` means something to the formula, language, while `I(age^2)`
 is protected. The coefficients are different to those we got before! However, the fits are the same:
 
```{r}
plot(fitted(fit), fitted(fitta))
```
...
matplot(age.grid,prob.bands, col="blue", lwd=c(2,1,1), ltw=c(1,2,2), type="l", ylim=c(0,.1))
...
Or we can use LOO cross-validation to select the smoothing paramater for us automatically:
